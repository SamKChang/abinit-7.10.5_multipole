...
!note also that one needs a blank after the second "!"
 ...
 real(dp), allocatable :: dmatpawu(:,:,:,:) !SET2NULL  ! dmatpawu(2*lpawu+1,2*lpawu+1,nsppol*nspinor,natpu)
 ...
 real(dp), allocatable :: jpawu(:)          !SET2NULL  ! jpawu(ntypat)
 ...
 real(dp) :: shiftk_orig(3,8)             ! original shifts given in input (changed in inkpts).

 real(dp), allocatable :: spinat(:,:)         !SET2NULL  ! spinat(3,natom)
 real(dp), allocatable :: tnons(:,:)          !SET2NULL  ! tnons(3,nsym)
 real(dp), allocatable :: upawu(:)            !SET2NULL  ! upawu(ntypat)
...
! This is for the parallelisation over fft
   ...
   ! Contains all the informations related to FFT distribution
...
! This is for the kpt/nsppol parallelisation
   ...
   integer, pointer :: proc_distrb(:,:,:)  => null()
...
! This is for Hartree-Fock's parallelisation
   ...
   integer, pointer :: distrb_hf(:,:,:) => null()
...
! These keywords are only used in algorithms using images of the cell
  ...
  integer :: unt_nlcc_derivs  ! unit number for non-linear core corrections
  integer :: unt_pspdat  ! unit number for pseudopotential data
...
!that particular file is only used for writing or for reading results, respectively.
  ...
  character(len=fnlen) :: fnameabo_ps_wfk
  character(len=fnlen) :: fnameabo_ae_onsite_wfk
  character(len=fnlen) :: fnameabo_ps_onsite_wfk
  ...
  character(len=fnlen) :: fnameabo_pawstat
...
!! A few of these subroutines are : hdr_init, hdr_update, hdr_free,
...
!  to know if time reversal can be employed
  ...
  !precision (single, double). It's needed in the GW coded 
  !so that I can run calculations in single or double precision
  !and dump the results with the same precision.
  !amu(ntypat) for DFPT. We can get it easily from the internal table but what happens if the user 
  !has specified a different value?
