...
!!
!! CHILDREN
!!      chkneu,inkpts,intagm,invacuum,smpbz,sort_int,timab,wrtout
!!
!! SOURCE
...
!scalars
 ...
 integer :: iprcch,ipsp,iscf,itypat,jj,kptopt,lpawu,marr,natom,nband1,nberry,mkpt
 integer :: niatcon,nimage,nkpt,npspalch,nqpt,nspinor,nsppol,nsym,ntypalch,ntypat,ntyppure,nkpt_hf
...
!Read ngfftdg(1:3)
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'fft_opt_lob',tread,'INT')
 if(tread==1) then
 ...
 call intagm(dprarr,intarr,jdtset,marr,3,string(1:lenstr),'ng',tread,'INT')
 if(tread==1) then
   write(message, '(a,a,a)' )&
...
&         'Action: check the values of ga_rules.'
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'chkgwcomp',tread,'INT')
...
&   'setting nfreqim = cd_customnimfrqs'
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'gw_use_pole_scr',tread,'INT')
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'gw_npoles',tread,'INT')
...
&         'Action: Correct this in your input file'
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'gw_reconst_scr',tread,'INT')
...
!Presently, rf?asr, rf?meth,rf?strs and rf?thrd are not used
 call intagm(dprarr,intarr,jdtset,marr,2,string(1:lenstr),'rf1atpol',tread,'INT')
 if(tread==1) dtset%rf1atpol(1:2)=intarr(1:2)

 call intagm(dprarr,intarr,jdtset,marr,3,string(1:lenstr),'rf1dir',tread,'INT')
 if(tread==1) dtset%rf1dir(1:3)=intarr(1:3)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'rf1elfd',tread,'INT')
 if(tread==1) dtset%rf1elfd=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'rf1phon',tread,'INT')
 if(tread==1) dtset%rf1phon=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,2,string(1:lenstr),'rf2atpol',tread,'INT')
 if(tread==1) dtset%rf2atpol(1:2)=intarr(1:2)

 call intagm(dprarr,intarr,jdtset,marr,3,string(1:lenstr),'rf2dir',tread,'INT')
 if(tread==1) dtset%rf2dir(1:3)=intarr(1:3)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'rf2elfd',tread,'INT')
 if(tread==1) dtset%rf2elfd=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'rf2phon',tread,'INT')
 if(tread==1) dtset%rf2phon=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,2,string(1:lenstr),'rf3atpol',tread,'INT')
 if(tread==1) dtset%rf3atpol(1:2)=intarr(1:2)

 call intagm(dprarr,intarr,jdtset,marr,3,string(1:lenstr),'rf3dir',tread,'INT')
 if(tread==1) dtset%rf3dir(1:3)=intarr(1:3)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'rf3elfd',tread,'INT')
 if(tread==1) dtset%rf3elfd=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'rf3phon',tread,'INT')
...
!real(dp) input variables
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'sciss',tread,'ENE')
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'freqsusin',tread,'DPR')
 if(tread==1) dtset%freqsusin=dprarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'freqsuslo',tread,'DPR')
 if(tread==1) dtset%freqsuslo=dprarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'optfreqsus',tread,'INT')
...
!  if(dtset%optdriver==RUNL_SIGMA) dtset%ecutsigx=dtset%ecut
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'soenergy',tread,'ENE')
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'getkss',tread,'INT')
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'irdkss',tread,'INT')
...
!real arrays
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'gw_eet',tread,'INT')
 if(tread==1) dtset%gw_eet=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'gw_eet_nband',tread,'INT')
 if(tread==1) dtset%gw_eet_nband=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'gw_eet_inclvkb',tread,'INT')
 if(tread==1) dtset%gw_eet_inclvkb=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'gw_eet_scale',tread,'ENE')
...
!Get array
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'getwf',tread,'INT')
 if(tread==1) then
   write(message, '(a,a,a)' )&
...
&   'Action: replace "getwf" by "getwfk" in your input file.'
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'accesswff',tread,'INT')
...
!variables for random positions in unit cell
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'iprcch',tread,'INT')
 if(tread==1) then
...
#ifdef HAVE_LOTF
   if(dtset%iprcch/=2)then
     iprcch=2
     dtset%iprcch=iprcch
     write(message, '(4a)' )&
&     'When ionmov==23, iprcch must be 2.',ch10,&
&     'Set iprcch to 2.',ch10
     MSG_COMMENT(message)
   end if
#else
   dtset%ionmov=12
...
&   'Set ionmov to 12.',ch10   
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'iprctfvw',tread,'INT')
...
!    Note that xcname is a 'key' variable : its value is stored in keyw at output of intagm
 ...
 if( dtset%ixc>=40 .and. dtset%ixc<=42 ) dtset%usefock=1 ! Hartree-Fock or hybrid functionals
 if( dtset%ixc>=41 .and. dtset%ixc<=42) dtset%xclevel=2 ! ixc for hybrids using GGA
#if defined HAVE_DFT_LIBXC
 if (dtset%ixc<0) then
   dtset%xclevel=1
   ii=-dtset%ixc/1000
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'ikhxc',tread,'INT')
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'npweps',tread,'INT')
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'npwsigx',tread,'INT')
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'nsheps',tread,'INT')
 if(tread==1) dtset%nsheps=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'nshsigx',tread,'INT')
...
!  dtset%pawecutdg=1.5_dp*dtset%ecut
 ...
 if (dtset%usedmft>0) then
   ...
   if(dtset%dmft_solv==4.or.dtset%dmft_solv==5) then
...
&     'Action: change  "useylm" value in your input file.'
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'ireadc',tread,'INT')
 if(tread==1) then
   write(message, '(a,a,a)' )&
...
!ACFD input variables
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'idyson',tread,'INT')
 if(tread==1) dtset%idyson=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'ndyson',tread,'INT')
 if(tread==1) dtset%ndyson=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'intexact',tread,'INT')
 if(tread==1) dtset%intexact=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'nbandsus',tread,'INT')
 if(tread==1) dtset%nbandsus=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'ldgapp',tread,'INT')
 if(tread==1) dtset%ldgapp=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'suskxcrs',tread,'INT')
...
!NON DEVLOP CASE
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'nloalg',tread,'INT')
 if(tread==1) then
   dtset%nloalg(1)=mod(intarr(1),10)
   dtset%nloalg(5)=intarr(1)/10
 end if
!DEVLOP CASE. Note that the fifth component is not used now.
!call intagm(dprarr,intarr,jdtset,marr,5,string(1:lenstr),'nloalg',tread,'INT')
!if(tread==1) then
...
!LOOP variables
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'nfreqsus',tread,'INT')
...
!Print variables
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'prtcml',tread,'INT')
 if(tread==1) dtset%prtcml=intarr(1)

 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'prtcs',tread,'INT')
...
!In case of a Berryphase calculation or orbital magnetism, put response = 1 
!in order to set istwfk = 1 at all k-points
 if ( (dtset%berryopt/=0) .OR. (dtset%orbmag /= 0) ) then
   response = 1
 end if
...
& dtset%kpt(:,1:nkpt),kptopt,dtset%kptnrm,dtset%kptrlatt,kptrlen,lenstr,nsym,&
...
!  Note that this number might be smaller than the one computed
!  by a slightly different formula in invars1
     dtset%nbandhf=dtset%nspinor*&
&     ((ceiling(zelect-1.0d-10)+1)/2 + ceiling( fband*natom - 1.0d-10 ))-1
!  More precisely, nbandhf = default value for nband (the number of occupied bands + 1) -1 = the number of occupied bands. 
   end if
...
!This is to be removed after a few version (from 5.4)
 call intagm(dprarr,intarr,jdtset,marr,ntypat,string(1:lenstr),'so_typat',tread_alt,'INT')
 if(tread_alt==1)then
   write(message, '(3a)' )&
