...
!!
!! COPYRIGHT
!! Copyright (C) 2014-2015 ABINIT group (AL)
...
!!      pawcprj_destroy,prep_getghc,prep_index_wavef_bandpp
...
subroutine chebfi(cg,dimcprj,dimffnl,dtfil,dtset,eig,enl,ffnl,gs_hamk,&
&           gsc,ikpt,kg,kinpw,mpi_enreg,natom,nband,npw,nspinor,&
&           ph3d,prtvol,resid,vlocal,&
&           vxctaulocal) ! optional argument
 ...
 use m_pawcprj, only : pawcprj_type, pawcprj_alloc, pawcprj_destroy, pawcprj_axpby, pawcprj_copy
...
!End of the abilint section
 ...
 integer,intent(in) :: dimffnl,ikpt,natom,nband,npw,prtvol,nspinor
 ...
 real(dp),intent(in) :: ffnl(npw,dimffnl,gs_hamk%lmnmax,gs_hamk%ntypat)
 real(dp),intent(in) :: kinpw(npw)
 real(dp),intent(inout) ::ph3d(2,npw,gs_hamk%matblk)
...
&   index_wavef_band)
 ...
 ! PAW init
 if(paw) then
   ABI_DATATYPE_ALLOCATE(cwaveprj, (natom,nspinor*nband_filter))
   ABI_DATATYPE_ALLOCATE(cwaveprj_next, (natom,nspinor*nband_filter))
   ABI_DATATYPE_ALLOCATE(cwaveprj_prev, (natom,nspinor*nband_filter))
   call pawcprj_alloc(cwaveprj,0,dimcprj)
   call pawcprj_alloc(cwaveprj_next,0,dimcprj)
   call pawcprj_alloc(cwaveprj_prev,0,dimcprj)
 ...
 if (dtset%paral_kgb == 0) then
   if(present(vxctaulocal))then
     call getghc(cpopt,cg_filter,cwaveprj,dimffnl,ffnl,dtfil%filstat,ghc_filter,gsc_filter,gs_hamk,gvnlc_filter,kg,&
&     kinpw,eval,mpi_enreg,natom,nband,npw,nspinor,dtset%paral_kgb,ph3d,prtvol,&
...
&     sij_opt,tim_getghc,0,vlocal,fock)
   ...
   if(present(vxctaulocal))then
     call prep_getghc(cg_filter,dimffnl,dtfil,gs_hamk,gvnlc_filter,ghc_filter,gsc_filter,ikpt,&
&     eval,nband,mpi_enreg,natom,npw,nspinor,dtset%paral_kgb,prtvol,&
&     sij_opt,vlocal,fock,cpopt,cwaveprj,.true.,vxctaulocal=vxctaulocal)
   else
     call prep_getghc(cg_filter,dimffnl,dtfil,gs_hamk,gvnlc_filter,ghc_filter,gsc_filter,ikpt,&
&     eval,nband,mpi_enreg,natom,npw,nspinor,dtset%paral_kgb,prtvol,&
&     sij_opt,vlocal,fock,cpopt,cwaveprj,.true.)
   end if
 end if
...
&     npw_filter, nactive, mpi_enreg, dimffnl, ffnl, ikpt, nspinor, kg, ph3d, dtset, dimcprj)
...
&       - gsc_filter_prev(:,shift:)
   ...
   if (dtset%paral_kgb == 0) then
     if(present(vxctaulocal))then
       call getghc(cpopt,cg_filter(:,shift:),cwaveprj(:,iactive:),dimffnl,ffnl,dtfil%filstat,&
&       ghc_filter(:,shift:),gsc_filter(:,shift:),gs_hamk,gvnlc_filter(:,shift:),kg,&
&       kinpw,eval,mpi_enreg,natom,nband,npw,nspinor,dtset%paral_kgb,ph3d,prtvol,&
&       sij_opt,tim_getghc,0,vlocal,fock,vxctaulocal=vxctaulocal)
     else
       call getghc(cpopt,cg_filter(:,shift:),cwaveprj(:,iactive:),dimffnl,ffnl,dtfil%filstat,&
...
&       sij_opt,tim_getghc,0,vlocal,fock)
     ...
     if(present(vxctaulocal))then
       call prep_getghc(cg_filter(:,shift:),dimffnl,dtfil,gs_hamk,gvnlc_filter(:,shift:),&
&       ghc_filter(:,shift:),gsc_filter(:,shift:),ikpt,&
&       eval,nband,mpi_enreg,natom,npw,nspinor,dtset%paral_kgb,prtvol,&
&       sij_opt,vlocal,fock,cpopt,cwaveprj(:,iactive:),.true.,vxctaulocal=vxctaulocal)
     else
       call prep_getghc(cg_filter(:,shift:),dimffnl,dtfil,gs_hamk,gvnlc_filter(:,shift:),&
&       ghc_filter(:,shift:),gsc_filter(:,shift:),ikpt,&
&       eval,nband,mpi_enreg,natom,npw,nspinor,dtset%paral_kgb,prtvol,&
&       sij_opt,vlocal,fock,cpopt,cwaveprj(:,iactive:),.true.)
     end if
   end if
 ...
 if(paw) then
   ...
   call pawcprj_destroy(cwaveprj_next)
   call pawcprj_destroy(cwaveprj_prev)
