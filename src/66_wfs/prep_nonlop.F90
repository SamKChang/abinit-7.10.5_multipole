...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2015 ABINIT group (FB,MT,GZ,MD,FDahm)
...
! *************************************************************************
 ...
 ABI_ALLOCATE(gsc_alltoall2,(2,ndatarecv*nspinor*(paw_opt/3)*bandpp))
 ...
 if(do_transpose .and. (bandpp/=1 .or. (bandpp==1 .and. mpi_enreg%paral_spinor==0.and.nspinortot==2)))then   
   ...
   if(signs==2)then
     ...
     if (paw_opt==3.or.paw_opt==4) then  
       ABI_ALLOCATE(gsc_alltoall1,(2,ndatarecv*nspinor*(paw_opt/3)*bandpp))
     endif  
   endif
 endif     
...
!    cwavef_alltoall(:, :)=cwavef_alltoall(:,index_wavef_band) ! This allowed dangerous side effects
   end if
...
&   phkxred,ph1d,ph3d_gather,ph3d_gather,signs,sij,gsc_alltoall2,&
&   tim_nonlop,ucvol,useylm,cwavef_alltoall2,gvnlc_alltoall2,use_gpu_cuda=usegpu)
...
!    if (paw_opt/=3) gvnlc_alltoall(:,index_wavef_band)=gvnlc_alltoall(:,:)    ! This allowed dangerous side effects
!    if (paw_opt==3.or.paw_opt==4) gsc_alltoall(:,index_wavef_band)=gsc_alltoall(:,:)   ! This allowed dangerous side effects
   end if
...
&         sendcountsloc,sdisplsloc,spaceComm,ier)
   ...
   call DCOPY(2*ndatarecv*nspinor*bandpp, gsc_alltoall2, 1, gsc, 1)
 end if
