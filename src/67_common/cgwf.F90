...
!!      pawcprj_copy,pawcprj_destroy,pawcprj_get,pawcprj_mpi_allgather
!!      pawcprj_put,projbd,smatrix,smatrix_k_paw,sqnorm_g,sym_pawcprj_kn,timab
!!      wrtout,xmpi_allgather
!!
!! SOURCE
...
&                ffnl,filnam_ds1,filstat,gsc,gs_hamk,fock,icg,igsc,ikpt,inonsc,&
&                isppol,kg_k,kinpw,mband,mcg,mcgq,mgsc,mkgq,mpi_enreg,&
&                mpw,natom,nband,nbdblock,nkpt,nline,nloalg,npw,npwarr,&
&                nspinor,nsppol,ortalg,paral_kgb,ph3d,prtvol,pwind,&
&                pwind_alloc,pwnsfac,pwnsfacq,quit,resid,subham,subovl,&
&                subvnl,tolrde,tolwfr,use_subovl,vlocal,wfoptalg,zshift,&
...
&                          pawcprj_get, pawcprj_mpi_allgather, pawcprj_destroy
 use m_hamiltonian, only : gs_hamiltonian_type
 use m_fock,        only : fock_type, fock_set_ieigen, fock_set_getghc_call
...
!Arguments ------------------------------------
 integer,intent(in) :: berryopt,chkexit,dimffnl,icg,igsc,ikpt,inonsc,isppol
 integer,intent(in) :: mband,mcg,mcgq,mgsc,mkgq,mpw,natom,nband,nbdblock,nkpt,nline
 integer,intent(in) :: npw,nspinor,nsppol,ortalg,paral_kgb,prtvol,pwind_alloc
 integer,intent(in) :: use_subovl,wfoptalg
 integer,intent(in) :: quit
 real(dp),intent(in) :: cpus,tolrde,tolwfr
 character(len=*),intent(in) :: filnam_ds1,filstat
...
!arrays
 ...
 real(dp),intent(in) :: cgq(2,mcgq),ffnl(npw,dimffnl,gs_hamk%lmnmax,gs_hamk%ntypat),kinpw(npw)
 ...
 real(dp),intent(inout) :: dphase_k(3) !vz_i
...
!integer :: jband ! used below in debugging
 integer :: job,mcg_q,me_distrb,ncpgr,nkpg,nblock,nproc_distrb,npw_k2
 ...
 real(dp) :: xnorm
 logical :: gen_eigenpb, finite_field,has_fock
 ...
 real(dp),allocatable :: gvnlc(:,:),gvnl_direc(:,:),gvnl_dummy(:,:),kpg(:,:)
...
! *********************************************************************
 ...
 has_fock = (associated(fock))
 fock_cg_typ = -1; if (has_fock) fock_cg_typ = fock%cg_typ
...
!Initializations and allocations
 ...
 if (gen_eigenpb.and.(inonsc==1))  then
   ...
   ABI_CHECK_ALLOC("out-of-memory in ghg_all")
 end if
...
!      Compute <g|H|c>
       ...
       if (has_fock.and.fock_cg_typ>=1) then
         ! By setting ieigen to the value iband, Fock contribution to the energy 
         ! for this orbital will be calculated.
         call fock_set_ieigen(fock,iband)
       end if
...
&           kinpw,eval,mpi_enreg,natom,1,npw,nspinor,paral_kgb,ph3d,prtvol,&
...
&           sij_opt,tim_getghc,0,vlocal,fock)
         ...
         call pawcprj_put(gs_hamk%atindx,cprj_band_srt,cprj_k,natom,iband,0,ikpt,1,isppol,&
&         mband,1,natom,1,mband,dimlmn,nspinor,nsppol,0,&
...
&           dtefield,ffnl,grad_berry,gs_hamk,iband,icg,ikpt,isppol,kg_k,&
&           mband,mcg,mcgq,mkgq,mpi_enreg,mpw,natom,nkpt,nloalg,&
&           npw,npwarr,nspinor,nsppol,pwind,ph3d,pwind_alloc,pwnsfac,pwnsfacq)
...
!$OMP PARALLEL DO 
         ...
         if(present(vxctaulocal))then
           ! Disable call to Fock if needed
           if (fock_cg_typ == 1) old_fockflag = fock_set_getghc_call(fock, 0)

           call getghc(cpopt,direc,cprj_dum,dimffnl,ffnl,filstat,gh_direc,gs_direc,gs_hamk,gvnl_direc,&
&           kg_k,kinpw,eval,mpi_enreg,natom,1,npw,nspinor,paral_kgb,ph3d,prtvol,&
&           sij_opt,tim_getghc,0,vlocal,fock,vxctaulocal=vxctaulocal)

           ! Restore previous value
           if (fock_cg_typ == 1) ii = fock_set_getghc_call(fock, old_fockflag)
...
&             kg_k,kinpw,eval,mpi_enreg,natom,1,npw,nspinor,paral_kgb,ph3d,prtvol,&
&             0,tim_getghc,0,vlocal,fock,vxctaulocal=vxctaulocal)
...
&                 gs_hamk%lmnmax,gs_hamk%matblk,gs_hamk%mgfft,mpi_enreg,natom,gs_hamk%nattyp,gs_hamk%ngfft,0,&
&                 nloalg,npw,nspinor,gs_hamk%ntypat,gs_hamk%phkxred,gs_hamk%ph1d,ph3d,gs_hamk%ucvol,&
...
!                icp1=dtefield%nband_occ*(ikptf-1)
                 ...
                 call pawcprj_put(gs_hamk%atindx,cprj_band_srt,cprj_direc,natom,iband,0,ikpt,1,isppol,&
&                 mband,1,natom,1,mband,dimlmn,nspinor,nsppol,0,&
&                 mpicomm=spaceComm_distrb,proc_distrb=mpi_enreg%proc_distrb)
...
&                 mpicomm=spaceComm_distrb,proc_distrb=mpi_enreg%proc_distrb)
                 ...
                 if (ikpt2 /= ikpt2f) then ! construct cprj_kb by symmetry
                   ...
                   call sym_pawcprj_kn(cprj_fkn,cprj_ikn,dtefield%atom_indsym,dimlmn,-1,gs_hamk%indlmn,&
...
&             ' deltae=',deltae,' < tolrde*',deold,' =>skip lines'
     ...
     if (xmpi_paral==0 .and. paral_kgb==0 .and. iband==nband .and. prtvol/=0) then
...
&     mpicomm=spaceComm_distrb,proc_distrb=mpi_enreg%proc_distrb)
 ...
 if (finite_field) then
   ...
   if (gs_hamk%usepaw /= 0) then
     ...
     call pawcprj_destroy(cprj_direc)
     call pawcprj_destroy(cprj_band_srt)
