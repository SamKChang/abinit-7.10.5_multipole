...
!!
!! COPYRIGHT
!! Copyright (C) 2006-2013 ABINIT group (BAmadon,AGerossier)
...
type, public :: plowannier_type
  ...
  type(orbital_type), pointer :: psichi(:,:,:)
  ! arrays of psichi
...
type, public :: orbital_type
  ...
  type(lorbital_type), pointer :: atom(:)
...
type, public :: operwan_type
  ...
  type(lorbital2_type), pointer :: atom(:,:)
...
type, public :: operwan_realspace_type
  ...
  type(operwan_type), pointer :: position(:,:)
...
!Prepare temporary PAW file if mkmem==0
 iorder_cprj=0
 call pawcprj_diskinit_r(cryst_struc%atindx1,dtset%natom,iorder_cprj,dtset%mkmem,dtset%natom,0,dimcprj,wan%nspinor,unpaw)
...
!*****************   in forlb.eig
 if(me.eq.0.and.abs(dtset%pawprtvol)>=3) then
   ...
   write(2010,*) dtset%mband,wan%nsppol,wan%nkpt,wan%nspinor,wan%bandi_wan,wan%bandf_wan
   write(2010,*) " For each k-point, eigenvalues for each band"
   write(2010,*) (dtset%wtk(ikpt)*facpara,ikpt=1,wan%nkpt)
   band_index=0
   do isppol=1,wan%nsppol
     ...
     do ikpt=1,wan%nkpt
       ...
       write(2010,*)  ikpt
       do iband=wan%bandi_wan,wan%bandf_wan
         write(2010, '(2i6,4x,f20.15)' ) iband-wan%bandi_wan+1,ikpt,eigen(iband+band_index)*2.d0
       end do
...
!!In operwan, energies level are stored (shifted with fermi level) in Hartree
 ...
 !! -------------------------------------------------------------
 !! Transform the Wannier operator in real space (in eV)
 !! 3) write operwan_realspace in a file unit 29
 !! -------------------------------------------------------------
   rewind(29)
   do isppol = 1,wan%nsppol
     do iatom1 = 1,wan%natom_wan
       do pos1 = 1,size(wan%nposition(iatom1)%pos,1)
         do il1 = 1,wan%nbl_atom_wan(iatom1)
           do iatom2 = 1,wan%natom_wan
             do pos2 = 1,size(wan%nposition(iatom2)%pos,1)
               do il2 = 1,wan%nbl_atom_wan(iatom2)
                 write(29) operwan_realspace(iatom1,iatom2)%position(pos1,pos2)%atom(il1,il2)%matl(:,:,isppol,1,1)
               end do
             end do
           end do
         end do
   ...
   write(message,'(2a)') ch10,&
&   '  == Write hamiltonian in real space Wannier function on unit 29 =='
   call wrtout(std_out,message,'COLL')
...
!!  BA?
 ...
 if (3==4.and.prtint .eq. 1 .and. dtset%plowan_realspace == 1 ) then
   open(25,file=trim(dtfil%filnam_ds(4))//'_inter')
   write(25,'(a,i0,a,F7.3)') "# nsppol = ",wan%nsppol," and acell = ",dtset%acell_orig(1,1)
   write(25,'(a)') "# Interaction between an orbital and another (in Hartree) : isppol iatom1 pos1 iproj1 iatom2 pos2 iproj2 value"
...
!                       end do
!                     end do
!                     write(25,'(i0,a,i0,a,i0,a,i0,a,i0,a,i0,a,i0,E15.6)') isppol," ",iatom1," ",pos1," ",wan%projector_wan(iatom1)%lproj(il1)," ",iatom2," ",pos2," ",wan%projector_wan(iatom2)%lproj(il2) ,sum
!                   end if
!                 end if
!               end do
!             end do
...
&             wan%kpt(3,ikpt)*(wan%nposition(1)%pos(1,3)-wan%nposition(iatom2)%pos(pos2,3))  )))
           ...
           write(25,'(i0,a,i0,a,i0,a,i0,a,i0,a,i0,a,i0,E15.6)') isppol,&
 ...
 if (dtset%plowan_realspace == 2) then
       ...
       do pos1 = 1,size(wan%nposition(iatom1)%pos,1)
         do il1 = 1,wan%nbl_atom_wan(iatom1)
           do iatom2 = 1,wan%natom_wan
             do pos2 = 1,size(wan%nposition(iatom2)%pos,1)
               do il2 = 1,wan%nbl_atom_wan(iatom2)
                 read(29) operwan_realspace(iatom1,iatom2)%position(pos1,pos2)%atom(il1,il2)%matl(:,:,isppol,1,1)
               end do
             end do
           end do
         end do
   ...
   write(message,'(2a)') ch10,&
&   '  == Read hamiltonian in real space Wannier function on unit 29 =='
   call wrtout(std_out,message,'COLL')
...
&   '   (The band structure is shifted by fermie =',fermie*27.211,' eV )'
   ...
   do iatom1 = 1,wan%natom_wan
     ...
     if (wan%nsppol .eq. 1) then
       open(25,file=trim(dtfil%filnam_ds(4))//"_BANDSTRUCT"//trim(x1))
       write(25,'(a,i0)') "#Wannier band structure for the atom ",iatom1
       ...
       open(25,file=trim(dtfil%filnam_ds(4))//"_BANDSTRUCTUP"//trim(x1))
       open(26,file=trim(dtfil%filnam_ds(4))//"_BANDSTRUCTDN"//trim(x1))
       write(25,'(a,i0,a)') "#Wannier band structure for the atom ",iatom1, " polarization up"
       write(26,'(a,i0,a)') "#Wannier band structure for the atom ",iatom1, " polarization down"
       do isppol = 1,wan%nsppol
         do ikpt = 1,wan%nkpt
           mat_writing = "#ikpt ="
           write(mat_writing2,'(i0)') ikpt
 ...
 if (plowan_computegreen .eq. 1) then
   if (dos .ge. 1) then ! compute partial DOS for l=dos
     open(25,file=trim(dtfil%filnam_ds(4))//"_dosfromgreen")
     write(25,'(a)') "#DOS function for the first atom computed with the Green function"
     write(25,'(a,i0,a,i0)') "# l = ",wan%latom_wan(1)%lcalc(dos)," bands ; nsppol = ",wan%nsppol
     write(25,'(a)') "# frequency, DOS, isppol" 
     if(wan%nsppol>=2) then
       open(250,file=trim(dtfil%filnam_ds(4))//"_dosfromgreen_b")
       write(250,'(a)') "#DOS function for the first atom computed with the Green function"
       write(250,'(a,i0,a,i0)') "# l = ",wan%latom_wan(1)%lcalc(dos)," bands ; nsppol = ",wan%nsppol
       write(250,'(a)') "# frequency, DOS, isppol" 
     endif
   end if

   if (dos .le. -1) then ! compute Hybri for l=|dos|
     open(25,file=trim(dtfil%filnam_ds(4))//"_hybridization")
     write(25,'(a)') "#Hybridization obtained from the green function for the bands selected"
     write(25,'(a,i0,a,i0)') "# l = ",wan%latom_wan(1)%lcalc(abs(dos))," bands ; nsppol = ",wan%nsppol
     write(25,'(a)') "#isppol, frequency, F(m=0), F(m=1), F(m=2) ..."
     if(wan%nsppol>=2)   then
     open(250,file=trim(dtfil%filnam_ds(4))//"_hybridization_b")
     write(250,'(a)') "#Hybridization obtained from the green function for the bands selected"
     write(250,'(a,i0,a,i0)') "# l = ",wan%latom_wan(1)%lcalc(abs(dos))," bands ; nsppol = ",wan%nsppol
     write(250,'(a)') "#isppol, frequency, F(m=0), F(m=1), F(m=2) ..."
     endif
   end if
...
&                     operwansquare(ikpt,isppol,index_l,index_c)*wan%wtk(ikpt)
     ...
     if (dos .ge. 1) then ! either we compute the DOS (-imaginary part/Pi of the green function in the Wannier basis)
       ...
       do isppol = 1,wan%nsppol
         ...
         if(isppol==1) write(25,'(F8.3,E15.6)') real(27.2101*wcurrent),sum/(27.2107*3.14159)
         if(isppol==2) write(250,'(F8.3,E15.6)') real(27.2101*wcurrent),sum/(27.2107*3.14159)
       end do
     ...
     ! attention a isppol ci dessus
         ...
         if(isppol==1) write(25,'(F10.3,a)') real(27.2107*wcurrent),trim(mat_writing)
         if(isppol==2) write(250,'(F10.3,a)') real(27.2107*wcurrent),trim(mat_writing)
       end do
     end if
     call destroy_operwan(wan,operwan)
     ABI_DATATYPE_DEALLOCATE(operwan)
...
!Local variables-------------------------------
 ...
 if (associated(wan%kpt)) then
   ABI_DEALLOCATE(wan%kpt)
 end if
 if (associated(wan%iatom_wan)) then
   ABI_DEALLOCATE(wan%iatom_wan)
 end if
 if (associated(wan%nbl_atom_wan)) then
   ABI_DEALLOCATE(wan%nbl_atom_wan)
 end if
 if (associated(wan%latom_wan)) then
   ABI_DATATYPE_DEALLOCATE(wan%latom_wan)
 end if
 if (associated(wan%nbproj_atom_wan)) then
   ABI_DEALLOCATE(wan%nbproj_atom_wan)
 end if
 if (associated(wan%projector_wan)) then
   ABI_DATATYPE_DEALLOCATE(wan%projector_wan)
 end if
 if (associated(wan%position)) then
   ABI_DEALLOCATE(wan%position)
 end if
 if (associated(wan%wtk)) then
   ABI_DEALLOCATE(wan%wtk)
 end if
 if (associated(wan%acell)) then
   ABI_DEALLOCATE(wan%acell)
 end if


 if (associated(wan%nposition)) then
   ABI_DATATYPE_DEALLOCATE(wan%nposition)
 end if
 if (associated(wan%psichi)) then
...
!    end do
  ...
  do isppol = 1,wan%nsppol
    do ikpt = 1,wan%nkpt
      call invsqrt_matrix(operwansquare(ikpt,isppol,:,:),wan%nspinor*wan%size_wan)
    end do
  end do
...
!Local variables----------------------------
  integer :: iatom1,iatom2,pos1,pos2,il1,il2,im1,im2,isppol,ikpt
  real(dp) :: sum
  character(len = 500) :: str1,str2

  open(26,file = trim(name))


  write(26,'(a)') '\documentclass[11pt,a4paper,landscape]{article}' 
 write(26,'(a)') '\usepackage[T1]{fontenc}'
  write(26,'(a)') '\usepackage{geometry,tabularx,graphicx}'
  write(26,'(a)') '\geometry{left=0.5cm,right=0.5cm}'

  write(26,'(a)') '\begin{document}'
  write(26,'(a)') '\noindent'

!  write(26,'(a,i0,a,F7.3,a)') "% ",wan%natom_wan," atom in a ",wan%acell(1)," cell"
!  write(26,'(a)') "% atom isppol proj"


do isppol = 1,wan%nsppol
  write(26,'(a)') '\begin{figure}'
  write(26,'(a)') '\resizebox{\linewidth}{!}{%'
  write(26,'(a)') '$ \left('
  ...
  write(26,'(a)') '\end{array} \right) $ }'
  
  if (name(len_trim(name)-2:len(trim(name))) == 'gen') then
    write(26,'(a,i0,a,F7.3,a)')     "\caption{Energy matrix in real space for isppol = ", &
&    isppol," in a ",wan%acell(1), " a.u. cell}"
  end if

  if (name(len_trim(name)-2:len(trim(name))) == 'occ') then
    write(26,'(a,i0,a,F7.3,a)')     "\caption{Occupation matrix in real space for isppol = ",&
&    isppol," in a ",wan%acell(1), " a.u. cell}"
  end if



  write(26,'(a)') '\end{figure}'
  write(26,'(a)') '\end{document}'
