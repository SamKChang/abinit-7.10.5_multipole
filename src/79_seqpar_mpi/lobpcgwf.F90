...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (FBottin,GZ,AR,MT,FDahm)
...
!Arguments ------------------------------------
 integer,intent(in) :: dimffnl,icg,igsc,ikpt,mcg,mgsc,natom,nband_k,nbdblock,npw_k,prtvol
...
!For communication
 ...
 !write(std_out,"(a,i0)")"blocksize: ",blocksize
...
&       sij_opt,tim_getghc,0,vlocal,fock)
     ...
     if(present(vxctaulocal))then
       call prep_getghc(cwavef,dimffnl,dtfil,gs_hamk,gvnlc,gwavef,swavef,ikpt,&
&       dum,blocksize,mpi_enreg,natom,npw_k,my_nspinor,dtset%paral_kgb,prtvol,&
&       sij_opt,vlocal,fock,cpopt,cprj_dum,already_transposed=.false.,vxctaulocal=vxctaulocal)
     else
       call prep_getghc(cwavef,dimffnl,dtfil,gs_hamk,gvnlc,gwavef,swavef,ikpt,&
&       dum,blocksize,mpi_enreg,natom,npw_k,my_nspinor,dtset%paral_kgb,prtvol,&
&       sij_opt,vlocal,fock,cpopt,cprj_dum,already_transposed=.false.)
     end if
   end if
   if(abs(dtset%timopt)==4) then
